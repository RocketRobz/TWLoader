// twpng.c - based on SFIL
// Major changes:
// - Improved support for certain types of PNG images.
...
static void _twl_read_png_file_fn(png_structp png_ptr, png_bytep data, png_size_t length)
{
	FILE *fp = (FILE *)png_get_io_ptr(png_ptr);
	fread(data, 1, length, fp);
}

static void _twl_read_png_buffer_fn(png_structp png_ptr, png_bytep data, png_size_t length)
{
...
static sf2d_texture *_twl_load_PNG_generic(const void *io_ptr, png_rw_ptr read_data_fn, sf2d_place place)
{
...
sf2d_texture *twl_load_PNG_file(const char *filename, sf2d_place place)
{
	...
	sf2d_texture *texture = _twl_load_PNG_generic((void *)fp, _twl_read_png_file_fn, place);
	fclose(fp);
	return texture;
...
sf2d_texture *twl_load_PNG_buffer(const void *buffer, sf2d_place place)
{
	...
	return _twl_load_PNG_generic((void *)&buffer_address, _twl_read_png_buffer_fn, place);
}
